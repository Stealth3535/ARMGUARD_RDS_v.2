...
class Command(BaseCommand):
    ...
    def handle_start_vpn(self):
        ...
        try:
            import subprocess
            # Hardcode interface name to prevent injection - never accept user input for interface
            interface_name = 'wg0'  # Static value only
    ...
    def handle_stop_vpn(self):
        ...
        try:
            import subprocess
            # Hardcode interface name to prevent injection - never accept user input
            interface_name = 'wg0'  # Static value only
            result = subprocess.run(
