...
def parse_qr_code(qr_data):
    ...
    # SECURITY FIX: Validate QR code format to prevent SQL injection
    import re
    if not re.match(r'^[A-Za-z0-9_-]{1,50}$', str(qr_data)):
        return {
            'success': False,
            'type': None,
            'data': {},
    ...
    try:
        # Look up QR code in database (Django ORM prevents SQL injection)
