...
class RateLimitMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        # Different rate limits for staff users instead of bypassing entirely
        if request.user.is_authenticated and request.user.is_staff:
            rate_limit = getattr(settings, 'RATELIMIT_STAFF_REQUESTS_PER_MINUTE', 300)  # Higher limit for staff
        else:
...
class AdminIPWhitelistMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        # Check both regular admin and Django admin paths
        admin_url = getattr(settings, 'DJANGO_ADMIN_URL', 'superadmin')
        if not (request.path.startswith('/admin/') or request.path.startswith(f'/{admin_url}/')):
            return None
