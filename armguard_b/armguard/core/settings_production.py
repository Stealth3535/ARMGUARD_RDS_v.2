...
# Security Headers - HTTPS/SSL - Enhanced for Military
...
CSRF_COOKIE_SAMESITE = 'Strict'  # Maximum protection
...
# Admin Security - IP Whitelist for admin interface
ADMIN_ALLOWED_IPS = config('ADMIN_ALLOWED_IPS', default='', cast=Csv())

# File Upload Security - Production hardening
FILE_UPLOAD_MAX_MEMORY_SIZE = 5242880  # 5MB
DATA_UPLOAD_MAX_MEMORY_SIZE = 5242880  # 5MB
FILE_UPLOAD_PERMISSIONS = 0o644  # Read-write for owner, read-only for others
FILE_UPLOAD_DIRECTORY_PERMISSIONS = 0o755  # Proper directory permissions

# Enhanced Password Validation - Military Grade
AUTH_PASSWORD_VALIDATORS = [
    ...
    },
    {
        ...
        'OPTIONS': {
            'min_length': 12,  # Military standard minimum
        }
    },
    {
...
# Database - PostgreSQL for production with enhanced security
    ...
    DATABASES = {
        'default': {
            ...
            'OPTIONS': {
                ...
                'options': '-c default_transaction_isolation=serializable'  # Highest isolation
            },
        }
    }
else:
...
# Cache Configuration - Redis for production (shared between workers)
if config('USE_REDIS_CACHE', default=True, cast=bool):
    CACHES = {
        'default': {
            ...
            'KEY_PREFIX': 'armguard_prod',
            'TIMEOUT': 300,  # 5 minute default
        }
    }
else:
    # Fallback to LocMemCache (WARNING: Not recommended for production)
    CACHES = {
        'default': {
            'BACKEND': 'django.core.cache.backends.locmem.LocMemCache',
            'LOCATION': 'armguard-cache-prod',
            'OPTIONS': {
                'MAX_ENTRIES': 1000,
                'CULL_FREQUENCY': 3,
            }
