        ...
        try:
            ...
            if not form.is_valid():
                ...
                for field, errors in form.errors.items():
                    if 'picture' in field.lower() or 'image' in field.lower():
                        for error in errors:
                            if 'file size' in str(error).lower() or 'invalid' in str(error).lower():
                                messages.error(request, f'File upload error for {field}: {error}')
                                break
            ...
            if form.is_valid():
                try:
                    ...
                    logger.error("Registration save error: %s", str(e), exc_info=True)
                    messages.error(request, f'Error during registration: {str(e)}')
        except Exception as e:
            logger.error("Registration form error: %s", str(e), exc_info=True)
            messages.error(request, 'Error processing form data. Please try again.')
    else:
        form = UniversalForm()
