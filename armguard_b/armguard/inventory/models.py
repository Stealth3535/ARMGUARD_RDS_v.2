...
class Item(models.Model):
    ...
    class Meta:
        ...
        verbose_name = 'Item'
        verbose_name_plural = 'Items'        # PERFORMANCE FIX: Add indexes for frequently queried fields
        ...
        indexes = [
            ...
            models.Index(fields=['item_type', 'status']),  # Composite index for common queries
            models.Index(fields=['status', 'condition']),  # Available items filtering
        ]
    ...
    def save(self, *args, **kwargs):
        """Override save to validate, generate ID if not set, and track status changes"""
        ...
        if is_status_change:
            try:
                ...
                AuditLog.objects.create(
                    ...
                    target_model='Item',
                    target_id=self.id,
                    target_name=f'{self.item_type} {self.serial}',
                    description=f'Item status changed: {old_status} \u2192 {self.status}',
                    changes={
