...
def _load_qr_image(item):
    ...
    try:
        ...
        qr_obj = (QRCodeImage.objects
                  .filter(qr_type='item', reference_id=item.id, is_active=True)
                  .exclude(qr_image='')
                  .first())
        if not qr_obj:
            return None
        try:
            qr_path = qr_obj.qr_image.path
        except Exception:
            qr_path = os.path.join(settings.MEDIA_ROOT, str(qr_obj.qr_image))
        if os.path.exists(qr_path):
            return Image.open(qr_path).convert('RGB')
        # Legacy media path fallback
        legacy = os.path.join(os.path.dirname(settings.MEDIA_ROOT),
                              'media', str(qr_obj.qr_image))
        if os.path.exists(legacy):
            return Image.open(legacy).convert('RGB')
...
def _build_tag(item) -> Image.Image:
    ...
    # ── Item number ────────────────────────────────────────────────────────
    num_text = f"{item.item_number:04d}" if item.item_number else '----'
    ...
    type_text  = item.get_item_type_display().upper()
