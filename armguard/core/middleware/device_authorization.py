    ...
    def process_request(self, request):
        ...
        if not self.is_device_authorized(device_fingerprint, ip_address, request.path, user):
            ...
            return HttpResponseForbidden(
                ...
                f'<p>Device ID: <code>{device_fingerprint[:8]}...</code></p>'
                f'<p>Timestamp: {timezone.now().strftime("%Y-%m-%d %H:%M:%S UTC")}</p>'
                '<p><em>Contact your system administrator for device authorization.</em></p>'
                '<hr><small>ArmGuard Military Security System</small>'
            )
