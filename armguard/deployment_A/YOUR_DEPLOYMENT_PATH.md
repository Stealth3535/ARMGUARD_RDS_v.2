# 🎯 YOUR RECOMMENDED DEPLOYMENT PATH
...
## ✅ RECOMMENDED: Use Ubuntu Auto-Deploy
...
✅ Detect hardware (CPU cores, RAM)
✅ Find your project at ~/ARMGUARD_RDS_v.2/armguard
✅ Calculate optimal workers
✅ Choose best database (likely PostgreSQL)
```
...
Use existing location? [Y/n]: Y <PRESS ENTER>  ← IMPORTANT! Use existing repo
...
```
✅ Install system packages (nginx, postgresql, redis, etc.)
✅ Create Python virtual environment
✅ Install Python dependencies
✅ Setup PostgreSQL database
✅ Generate Django configuration (.env file)
...
~/ARMGUARD_RDS_v.2/armguard/
    ...
    ├── db.sqlite3 or PostgreSQL DB   → Database
    └── static/                       → Static files

/etc/systemd/system/
    ├── gunicorn-armguard.service     → App server service
    └── daphne-armguard.service       → WebSocket service

/etc/nginx/sites-available/
    └── armguard                      → Nginx configuration

/etc/ssl/certs/ or ~/.local/share/mkcert/
    └── armguard certificates         → SSL certificates
```

### **Services Running:**
```
Port 80/443  → Nginx (reverse proxy)
Port 8000    → Gunicorn (Django app)
Port 8001    → Daphne (WebSockets)
Port 5432    → PostgreSQL  (database)
Port 6379    → Redis (cache/channels)
```
...
### **During Deployment:**
1. **When asked "Use existing location?"** → Answer **Y** (yes)
   - This lets you easily update with `git pull`
   - No need to copy files

2. **SSL Certificate Choice:**
   - **mkcert** → For LAN-only (192.168.x.x)
   - **Let's Encrypt** → For public domain (requires DNS)
...
**Then follow the prompts and you'll be deployed in ~10 minutes!** 🚀
...
- **Python package errors** → Check internet connection
