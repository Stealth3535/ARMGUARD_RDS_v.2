...
# Check for integrated network setup (preferred method)
if [ -f "$SCRIPT_DIR/../../02_config.sh" ]; then
    echo -e "${GREEN}✓ Integrated network setup available${NC}"
    INTEGRATED_NETWORK_AVAILABLE=true
else
    echo -e "${YELLOW}⚠ Integrated network setup not found${NC}"
    INTEGRATED_NETWORK_AVAILABLE=false
fi
...
case $NETWORK_TYPE in
    lan)
        ...
        if [ "$INTEGRATED_NETWORK_AVAILABLE" = true ]; then
            echo -e "${CYAN}⚡ Using integrated network configuration...${NC}"
            echo -e "${WHITE}For advanced LAN network setup, run: cd ../../ && ./02_config.sh${NC}"
            echo -e "${YELLOW}Note: This enterprise script uses basic SSL setup. For advanced features, use modular deployment.${NC}"
            # Basic mkcert setup
            if [ -f "$SCRIPT_DIR/install-mkcert-ssl.sh" ]; then
                bash "$SCRIPT_DIR/install-mkcert-ssl.sh" "$DEFAULT_DOMAIN"
            else
                echo -e "${RED}⚠ Basic SSL setup not available. Please run modular deployment.${NC}"
            fi
        elif [ -f "$SCRIPT_DIR/../network_setup/setup-lan-network.sh" ]; then
            bash "$SCRIPT_DIR/../network_setup/setup-lan-network.sh"
        elif [ -f "$SCRIPT_DIR/install-mkcert-ssl.sh" ]; then
            bash "$SCRIPT_DIR/install-mkcert-ssl.sh" "$DEFAULT_DOMAIN"
        else
            echo -e "${RED}❌ No SSL setup method available${NC}"
            echo -e "${WHITE}Please use modular deployment: cd ../../ && ./02_config.sh${NC}"
        fi
    ...
    wan)
        ...
        if [ "$INTEGRATED_NETWORK_AVAILABLE" = true ]; then
            echo -e "${CYAN}⚡ Advanced WAN SSL setup available in integrated deployment${NC}"
            echo -e "${WHITE}For full WAN configuration (ACME), run: cd ../../ && ./02_config.sh${NC}"
            echo -e "${RED}❌ Enterprise script does not support WAN SSL setup${NC}"
            echo -e "${WHITE}Please use modular deployment for WAN/public configurations.${NC}"
        elif [ -f "$SCRIPT_DIR/../network_setup/setup-wan-network.sh" ]; then
            bash "$SCRIPT_DIR/../network_setup/setup-wan-network.sh"
        else
            echo -e "${RED}❌ WAN SSL setup not available${NC}"
            echo -e "${WHITE}Please use modular deployment: cd ../../ && ./02_config.sh${NC}"
        fi
        record_phase "SSL Setup" "MANUAL_REQUIRED (WAN/ACME)"
        ;;
    hybrid)
        echo -e "${YELLOW}Setting up Hybrid SSL (LAN + WAN)...${NC}"
        if [ "$INTEGRATED_NETWORK_AVAILABLE" = true ]; then
            echo -e "${CYAN}⚡ Advanced hybrid network setup available in integrated deployment${NC}"
            echo -e "${WHITE}For full hybrid configuration, run: cd ../../ && ./02_config.sh${NC}"
            echo -e "${RED}❌ Enterprise script does not support hybrid SSL setup${NC}"
            echo -e "${WHITE}Please use modular deployment for hybrid network configurations.${NC}"
        else
        ...
        record_phase "SSL Setup" "MANUAL_REQUIRED (Hybrid)"
        ;;
esac
...
if [ "$ENABLE_FIREWALL" = "yes" ]; then
    ...
    if [ "$INTEGRATED_NETWORK_AVAILABLE" = true ]; then
        echo -e "${CYAN}⚡ Advanced firewall configuration available in integrated deployment${NC}"
        echo -e "${WHITE}For comprehensive firewall setup, run: cd ../../ && ./02_config.sh${NC}"
        echo -e "${YELLOW}Using basic enterprise firewall setup...${NC}"
        
        # Basic enterprise firewall setup
        ...
        case "$NETWORK_TYPE" in
            "lan")
                ufw allow 8443/tcp comment 'LAN HTTPS'
                echo -e "${GREEN}✓ Basic LAN firewall configured${NC}"
                ;;
            "hybrid"|"wan")  
                ufw allow 80/tcp comment 'HTTP'
                ufw allow 443/tcp comment 'HTTPS'
                echo -e "${GREEN}✓ Basic WAN firewall configured${NC}"
                ;;
        esac
        
        ufw --force enable
        echo -e "${YELLOW}Note: For advanced firewall features (fail2ban, rate limiting), use modular deployment.${NC}"
        
    elif [ "$NETWORK_TYPE" = "hybrid" ] && [ -f "$SCRIPT_DIR/../network_setup/configure-firewall.sh" ]; then
